...
class Transaction(Base):
    ...
    status = Column(Enum(TransactionStatus), nullable=False, default=TransactionStatus.PROPOSED)
    ...
    # Bot voting system (for TRADE transactions) - VETO-BASED SYSTEM
    voting_bots = Column(JSON, default=[])  # List of bot IDs eligible to vote (all league bots except participants)
    votes = Column(JSON, default={})  # Dict: {"bot_id": {"vote": "PASS"/"VETO", "comment": "...", "reason": "..."}}
    voting_ends_at = Column(DateTime(timezone=True))  # When voting period ends
    veto_votes_required = Column(Integer, default=1)  # Veto votes needed to block trade (default: 1 veto blocks)
    ...
    @property
    def pass_votes(self):
        """Count pass votes (allow trade)."""
        if not self.votes:
            return 0
        return len([v for v in self.votes.values() if v.get("vote") == "PASS"])
    
    @property
    def veto_votes(self):
        """Count veto votes (block trade)."""
        if not self.votes:
            return 0
    ...
    def votes_received(self):
        """Calculate total votes received (PASS + VETO)."""
        return self.pass_votes + self.veto_votes
    ...
    def add_vote(self, bot_id, vote, comment=None, reason=None):
        """
        Add a vote from a bot in veto-based system.
        
        Args:
            bot_id: Bot ID voting
            vote: "PASS" (allow trade) or "VETO" (block trade)
            comment: Optional comment from the bot
            reason: AI-generated reason for the vote based on personality
        """
        if vote not in ["PASS", "VETO"]:
            raise ValueError(f"Invalid vote: {vote}. Must be PASS or VETO")
    ...
    def _check_voting_complete(self):
        """Check if voting is complete and update status in veto-based system."""
        if not self.voting_in_progress:
            return
        
        # Check if veto threshold reached
        if self.veto_votes >= self.veto_votes_required:
            self.status = TransactionStatus.VETOED
            return
        
        # Check if voting period has ended (all bots voted or time expired)
        if self.voting_complete:
            # If we reach here, veto threshold not reached and voting complete
            self.status = TransactionStatus.PASSED
    ...
    def to_sleeper_format(self):
        ...
        base = {
            ...
            "consenter_ids": [bot_id for bot_id, vote_data in (self.votes.items() if self.votes else []) 
                             if vote_data.get("vote") == "PASS"],
        }
