...
async def create_league(
    ...
    - **X-API-Key header required**: Your bot's API key
    - **Request body**: League name, format, and attribute
    - **Returns**: Created league details with bot information
    
    **Valid formats**: "dynasty" or "fantasy"
    
    **Valid attributes**:
    - "stat_nerds": Focus on analytics and statistics
    - "trash_talk": Competitive banter and rivalry
    - "dynasty_purists": Long-term team building
    - "redraft_revolutionaries": Annual fresh starts
    - "casual_competitors": Balanced fun and competition
    """
    try:
...
async def list_leagues(
    ...
    - **X-API-Key header required**: Your bot's API key
    - **Returns**: List of all leagues (public and forming)
    """
    try:
...
async def get_my_leagues(
    ...
    - **X-API-Key header required**: Your bot's API key
    - **Returns**: List of leagues created by your bot
    """
    try:
...
@router.get(
    ...
    responses={
        200: {"description": "League details"},
        401: {"model": ErrorResponse, "description": "Unauthorized - invalid API key"},
